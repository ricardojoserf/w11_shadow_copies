...
class ShadowCopyManager
{
    ...
    static Guid ExtractSnapshotIdFromPath(string devicePath)
    {
        ...
        try
        {
            ...
            Guid guidNull = GUID_NULL;
            hr = pBackup.Query(ref guidNull, VSS_OBJECT_TYPE.VSS_OBJECT_NONE, VSS_OBJECT_TYPE.VSS_OBJECT_SNAPSHOT, out pEnum);
    ...
    static void ListShadowCopies()
    {
        ...
        if (hr != 0 && hr != 1)
        {
            Console.WriteLine($"Error initializing COM. HRESULT: 0x{hr:X8}");
            return;
        }
    ...
    static void Main(string[] args)
    {
        try
        {
            ...
            if (args.Length == 0)
            {
                Console.WriteLine("Usage: program.exe <command> [args]");
                Console.WriteLine("Commands:");
                Console.WriteLine("  create              - Create shadow copy of C:\\");
                Console.WriteLine("  delete <path>       - Delete shadow copy");
                Console.WriteLine("  list                - List all shadow copies");
                return;
            }
            ...
            else if (command == "delete")
            {
                if (args.Length < 2)
                {
                    Console.WriteLine("ERROR: Delete command requires device path");
                    Console.WriteLine("Example: program.exe delete \\\\?\\GLOBALROOT\\Device\\HarddiskVolumeShadowCopy8");
                    return;
                }
            ...
            else
            {
                ...
                Console.WriteLine("Valid commands: create, delete, list");
            }
        }
        catch (Exception ex)
        {
