...
def extract_snapshot_id_from_path(device_path : String) : VssLib::GUID?
  ...
  begin
    ...
    puts "No shadow copy found with device path: #{device_path}"
    return nil
...
def delete_shadow_copy(device_path : String) : Bool
  ...
  begin
    ...
    if hr != 0
      puts "DeleteSnapshots error. HRESULT: 0x#{hr.to_s(16)}"
      case hr
      when 0x80042308
        puts "  -> VSS_E_VOLUME_NOT_SUPPORTED_BY_PROVIDER"
      when 0x8004230C
        puts "  -> VSS_E_BAD_STATE"
      when 0x80042316
        puts "  -> VSS_E_OBJECT_NOT_FOUND"
      end
      return false
    end
    ...
    if deleted_snapshots > 0
      ...
      puts "Non-deleted snapshot ID: {#{guid_to_string(nondeleted_snapshot_id)}}"
      return false
    end
...
def main
  ...
  if ARGV.size < 1
    puts "Usage: delete <device_path>"
    puts "Example: delete \\\\?\\GLOBALROOT\\Device\\HarddiskVolumeShadowCopy8"
    return
  end
